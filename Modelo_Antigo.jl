...
#https://blog.shopfisio.com.br/como-calcular-o-gasto-calorico/
#using CSV
#using DataFrames
...
@objective(model, Max, sum(1e-4*quantidade.^2))
#@constraint(model, meta_calorica,  quantidade_alimento[i]*sum(macros[i,1:3]) - basa ≤ 1e-2)

@constraint( model, limitante_comida[i = 1:alimentos],
    quantidade_alimento[i]/100 ≤ quantidade[i] * lim_sup)

@constraint( model, limitante_comida_inf[i = 1:alimentos],
    quantidade_alimento[i]/100 ≥ quantidade[i] * 1e-4)

@constraint(  model,  meta_carb,
    -1e-4 ≤  sum(quantidade_alimento[i] * macros[i, 1] for i = 1:alimentos)/100 - carb_cal ≤  1e-4)

@constraint(model, meta_prot,
    -1e-4 ≤ sum(quantidade_alimento[i] * macros[i, 2] for i = 1:alimentos)/100 - prot_cal ≤ 1e-4)

@constraint( model, meta_gord,
    -1e-4 ≤ sum(quantidade_alimento[i] * macros[i, 3] for i = 1:alimentos)/100 - gord_cal ≤ 1e-4)
